...
export const zohocrmProvider: IntegrationProviderDef = {
  id: 'zohocrm',
  categoryId: 'crm',
  name: 'Zoho CRM',
  description: 'Sync data with Zoho CRM.',
  icon: <img src="https://www.google.com/s2/favicons?domain=zoho.com&sz=64" alt="Zoho CRM Logo" className="w-8 h-8 object-contain rounded-md" />,
  getDefaultState: () => ({
    tenantId: '',
    clientId: '',
    clientSecret: '',
  ...
  getWizardSteps: ({ state, updateState, integration, testResult, setTestResult, setIsTesting }) => [
    {
      ...
      onNext: async () => {
        ...
        setTestResult(null);
        try {
          const res = await api.post(`/admin/integrations/zohocrm/test`, {
            tenantId: state.tenantId,
