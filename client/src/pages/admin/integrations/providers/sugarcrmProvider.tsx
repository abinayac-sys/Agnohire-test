...
export const sugarcrmProvider: IntegrationProviderDef = {
  id: 'sugarcrm',
  categoryId: 'crm',
  name: 'SugarCRM',
  description: 'Connect SugarCRM to AgnoHire.',
  icon: <img src="https://www.google.com/s2/favicons?domain=sugarcrm.com&sz=64" alt="SugarCRM Logo" className="w-8 h-8 object-contain rounded-md" />,
  getDefaultState: () => ({
    tenantId: '',
    clientId: '',
    clientSecret: '',
  ...
  getWizardSteps: ({ state, updateState, integration, testResult, setTestResult, setIsTesting }) => [
    {
      ...
      onNext: async () => {
        ...
        setTestResult(null);
        try {
          const res = await api.post(`/admin/integrations/sugarcrm/test`, {
            tenantId: state.tenantId,
