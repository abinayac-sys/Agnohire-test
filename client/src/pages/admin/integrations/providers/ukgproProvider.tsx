...
export const ukgproProvider: IntegrationProviderDef = {
  id: 'ukgpro',
  categoryId: 'hrms',
  name: 'UKG Pro',
  description: 'Ultimate Kronos Group Integration.',
  icon: <img src="https://www.google.com/s2/favicons?domain=ukg.com&sz=64" alt="UKG Pro Logo" className="w-8 h-8 object-contain rounded-md" />,
  getDefaultState: () => ({
    tenantId: '',
    clientId: '',
    clientSecret: '',
  ...
  getWizardSteps: ({ state, updateState, integration, testResult, setTestResult, setIsTesting }) => [
    {
      ...
      onNext: async () => {
        ...
        setTestResult(null);
        try {
          const res = await api.post(`/admin/integrations/ukgpro/test`, {
            tenantId: state.tenantId,
