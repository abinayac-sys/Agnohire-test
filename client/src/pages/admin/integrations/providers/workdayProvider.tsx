...
export const workdayProvider: IntegrationProviderDef = {
  id: 'workday',
  categoryId: 'hrms',
  name: 'Workday',
  description: 'Connect Workday HR and Finance.',
  icon: <img src="https://www.google.com/s2/favicons?domain=workday.com&sz=64" alt="Workday Logo" className="w-8 h-8 object-contain rounded-md" />,
  getDefaultState: () => ({
    tenantId: '',
    clientId: '',
    clientSecret: '',
  ...
  getWizardSteps: ({ state, updateState, integration, testResult, setTestResult, setIsTesting }) => [
    {
      ...
      onNext: async () => {
        ...
        setTestResult(null);
        try {
          const res = await api.post(`/admin/integrations/workday/test`, {
            tenantId: state.tenantId,
