...
export const bamboohrProvider: IntegrationProviderDef = {
  id: 'bamboohr',
  categoryId: 'hrms',
  name: 'BambooHR',
  description: 'BambooHR HRIS integration.',
  icon: <img src="https://www.google.com/s2/favicons?domain=bamboohr.com&sz=64" alt="BambooHR Logo" className="w-8 h-8 object-contain rounded-md" />,
  getDefaultState: () => ({
    tenantId: '',
    clientId: '',
    clientSecret: '',
  ...
  getWizardSteps: ({ state, updateState, integration, testResult, setTestResult, setIsTesting }) => [
    {
      ...
      onNext: async () => {
        ...
        setTestResult(null);
        try {
          const res = await api.post(`/admin/integrations/bamboohr/test`, {
            tenantId: state.tenantId,
