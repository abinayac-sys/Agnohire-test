...
export const hubspotProvider: IntegrationProviderDef = {
  id: 'hubspot',
  categoryId: 'crm',
  name: 'HubSpot CRM',
  description: 'Inbound marketing and CRM integration.',
  icon: <img src="https://www.google.com/s2/favicons?domain=hubspot.com&sz=64" alt="HubSpot CRM Logo" className="w-8 h-8 object-contain rounded-md" />,
  getDefaultState: () => ({
    tenantId: '',
    clientId: '',
    clientSecret: '',
  ...
  getWizardSteps: ({ state, updateState, integration, testResult, setTestResult, setIsTesting }) => [
    {
      ...
      onNext: async () => {
        ...
        setTestResult(null);
        try {
          const res = await api.post(`/admin/integrations/hubspot/test`, {
            tenantId: state.tenantId,
