...
export const salesforceProvider: IntegrationProviderDef = {
  id: 'salesforce',
  categoryId: 'crm',
  name: 'Salesforce CRM',
  description: 'Connect Salesforce CRM for candidate data sync.',
  icon: <img src="https://www.google.com/s2/favicons?domain=salesforce.com&sz=64" alt="Salesforce CRM Logo" className="w-8 h-8 object-contain rounded-md" />,
  getDefaultState: () => ({
    tenantId: '',
    clientId: '',
    clientSecret: '',
  ...
  getWizardSteps: ({ state, updateState, integration, testResult, setTestResult, setIsTesting }) => [
    {
      title: 'Salesforce CRM Credentials',
      ...
      onNext: async () => {
        ...
        setTestResult(null);
        try {
          const res = await api.post(`/admin/integrations/salesforce/test`, {
            tenantId: state.tenantId,
