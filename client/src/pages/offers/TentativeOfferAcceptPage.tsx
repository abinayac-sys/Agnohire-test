...
export function TentativeOfferAcceptPage() {
  ...
  useEffect(() => {
    async function loadOffer() {
      try {
        const info = await offerApi.getTentativeOffer(token);
        setOfferInfo({
  ...
  async function handleAccept() {
    setSubmitting(true);
    try {
      await offerApi.acceptTentativeOffer(token);
      setPhase('success');
    } catch (e) {
      setErrorMsg(apiErrorMessage(e));
      setPhase('error');
    ...
    return (
      <Shell>
        <div className="flex flex-col items-center gap-3 text-text-secondary">
          ...
          <p className="text-sm font-medium">Loading tentative offer details…</p>
        </div>
      </Shell>
    );
  }
    ...
    return (
      <Shell>
        <Centered
          ...
          body="This tentative offer has already been accepted. Check your email for a separate link to upload your required documents."
        />
      </Shell>
    );
  }
    ...
    return (
      <Shell>
        <Centered
          ...
          title="Tentative Offer Accepted Successfully"
          body="Thank you for accepting the tentative offer. We'll email you a separate link shortly to upload your required joining documents."
        />
      </Shell>
    );
  }
      ...
      <div className="space-y-6 text-center">
        ...
        <div>
          <h1 className="font-heading text-2xl font-bold text-text-primary">
            Review Your Tentative Job Offer
          </h1>
          <p className="mt-1.5 text-sm text-text-secondary">
            Hi {offerInfo?.candidateName}, we are excited to have you clear the selection process!
          </p>
        </div>
