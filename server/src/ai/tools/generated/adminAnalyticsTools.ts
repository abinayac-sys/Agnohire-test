...
ToolRegistry.register({
  ...
  description: 'Auto-generated tool for GET /api/admin-analytics/overview',
  parameters: {
    type: 'object',
    properties: {
  ...
  execute: async (args, ctx, _req) => {
    ...
    let url = `http://localhost:${port}/api/admin-analytics/overview`;
...
ToolRegistry.register({
  ...
  category: 'adminAnalytics',
  allowedRoles: [ROLES.SUPERADMIN],
  description: 'Auto-generated tool for GET /api/admin-analytics/health',
  parameters: {
    type: 'object',
    properties: {
      body: { type: 'object', description: 'Request payload' },
  ...
  execute: async (args, ctx, _req) => {
    const port = process.env.PORT || 4000;
    let url = `http://localhost:${port}/api/admin-analytics/health`;
